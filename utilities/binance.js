...
const normalizePriceAndQuantity = (exchangeInfo, orderData, type) => {
    ...
    if (type === 'tp') {
        ...
        throw new Error('undefined order type passed to normalizePriceAndQuantity function');
    }
    for (let i = 0; i < exchangeInfo.symbols.length; i++) {

        if (exchangeInfo.symbols[i].symbol == orderData.symbol) {

            var stepSize = Math.log10(1 / parseFloat(exchangeInfo.symbols[2].filters[2].stepSize));
            var tickSize = Math.log10(1 / parseFloat(exchangeInfo.symbols[0].filters[0].tickSize));
            modifiedQuantity = Math.floor(orderData.quantity * 10 ** stepSize) / 10 ** stepSize;
...
const palceInitialOrder = (orderData) => {
    ...
    if (orderData.type == 1) {// in case of take profit 
...
const cancelPostion = (orderData) => {
    ...
    binanceRest.cancelOrder({
        ...
        then((res1) => {
            //Normalize Quantity
            var normalizedQuantity = normalizePriceAndQuantity(res1, orderData, 'tp').modifiedQuantity;//its not tp but we just need theNormalize Quantity
